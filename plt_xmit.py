...
date0='2021-08-25'; date1='2022-06-06'
...
t0=datetime(2021,8,21) ; t1=datetime(2022, 6, 6)
        ...
        # if meta.alt_name[i]=='QAS_U':
            ...
            if meta.alt_name[i]=='SWC' or meta.alt_name[i]=='JAR':
                ...
                cols=['time','seconds_since_1990','airpressureminus1000','temperature','relativehumidity','windspeed','winddirection',
                      'SWU','SWD',
                     'LW Downward','LW Upward','TemperatureRadSensor','SR_1','SR_2','solar?',
            ...
            if drop:
                ...
                t0=datetime(2021, 8, 21) ; t1=datetime(2022, 6, 3)
            ...
            for sensor_level in sensor_levels:
                ...
                df["SR_"+sensor_level]=df["SR_"+sensor_level][0]-df["SR_"+sensor_level]
                df["SR_"+sensor_level][df["SR_"+sensor_level]<-0.1]=np.nan
                df["SR_"+sensor_level][df["SR_"+sensor_level]>2]=np.nan
            ...
            if do_plot:
                ...
                if N>0:
                    ...
                    ax[cc].plot(df.SR_1,'.',label='SR A')
                    ax[cc].plot(df.SR_2,'.',label='SR B')
                    ax[cc].get_xaxis().set_visible(False)
                    ax[cc].legend()
                    ax[cc].set_xlim(t0,t1)
                    cc+=1                
