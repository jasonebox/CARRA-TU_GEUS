...
# cols=['SR_A','SR_B','IceHeight','TemperatureIce1','TemperatureIce2','TemperatureIce3','TemperatureIce4','TemperatureIce5','TemperatureIce6','TemperatureIce7','TemperatureIce8','TiltX','TiltY','TimeGPS','Latitude','Longitude','Altitude','HDOP','currents','Battery','AirPressureMinus1000','Temperature','RelativeHumidity','WindSpeed','WindDirection','InstantDataTerminator']
        ...
        t0=datetime(2022,1,1) ; t1=datetime(2022, 10, 1)
        ...
        # tx
        fn='/Users/jason/0_dat/aws-l3/tx/'+site+'/'+site+'_hour.csv'
        print(fn)
        df=pd.read_csv(fn)
        print(df.columns)
        ...
        df['rh_i'][df['rh_i']>105]=np.nan
        df['rh_i'][df['rh_i']<30]=np.nan
        # df.p_i[df.p_i>1000]=np.nan
        # df.p_i[df.p_i<600]=np.nan
...
# #%%
        ...
        if site=='SWC':
            df=adjuster(site,df,['winddirection'],2020,8,9,'rotate',2022,8,1,'wind dir box 180deg off',180)
